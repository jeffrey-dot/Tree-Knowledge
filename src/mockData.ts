import type { NodeDetailMock, RetrievalHit, TreeNode } from "./types";
...
export const nodeDetails: Record<string, NodeDetailMock> = {
  root: {
    content: [
      ...
      "MVP 第一优先级是证明树形上下文边界正确：当前节点只能继承根、父链和自己；兄弟分支默认排除；发送给 LLM 的上下文必须可检查。",
    ],
  },
  product: {
    content: [
  ...
  context: {
    content: [
      "自动上下文只允许三类内容：根节点、从根到当前节点的父链、当前节点自身。兄弟节点、叔伯节点、归档节点和无关分支都不能自动进入 prompt。",
      "全局搜索和网页结果可以显示给用户，但进入上下文前必须明确标注来源，并由用户显式选择。这样用户能理解模型为什么看到了某段内容，也能看见哪些分支被排除了。",
      "上下文预览不是调试工具，而是核心使用体验的一部分。这个产品的信任基础就是边界可解释。",
    ],
  },
  ui: {
    content: [
  ...
  retrieval: {
    content: [
      "检索默认应该优先当前节点，其次是直接父节点，再往上是更早的祖先节点。确认过的当前引用可以排在全局知识前面。",
