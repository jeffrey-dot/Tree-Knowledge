...
- Tauri 2 作为桌面端外壳。
- 默认不再加载 seed/mock 树数据；应用从知识库总览开始。
- 当前可在前端会话内新建多个独立知识库，并打开其中一个进入树形工作台；SQLite 持久化与打开本地数据库中的知识库尚未接入。
...
当前版本用于展示桌面端主工作流：知识库总览、新建独立知识库、打开单个知识库进入全屏树画布、父链高亮、点击节点打开详情弹窗、悬浮节点展开可创建的子主题，以及手动输入问题生成新节点。
